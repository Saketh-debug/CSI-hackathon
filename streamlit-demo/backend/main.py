...
# ═══════════════════════════════════════════════════════════
# ENDPOINT: GET /api/walkability
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
@app.get("/api/walkability")
def get_walkability():
    """
    Returns walkability stats + URL to walkability PNG overlay.
    """
    from services.surface_temp import fetch_lst_raster
    from services.ndvi import fetch_ndvi_raster
    ...
    try:
        ...
        walk_data = compute_walkability_grid(lst_data, ndvi_data)
    except Exception as e:
        raise HTTPException(status_code=503, detail=f"Walkability service error: {e}")

    stats = walk_data.get("stats", {})
    ts = int(walk_data.get("timestamp", time.time()))

    return {
        "bounds": walk_data["bounds"],
        "stats": {
            "avg_score": stats.get("avg_score", 0),
            "pct_good": stats.get("pct_good", 0),
            "pct_moderate": stats.get("pct_moderate", 0),
            "pct_poor": stats.get("pct_poor", 0),
            "max_uv": stats.get("max_uv", 0),
            "avg_aqi": stats.get("avg_aqi", 0),
            "avg_slope_deg": stats.get("avg_slope_deg", 0),
        },
        "best_hours": walk_data.get("best_hours", []),
        "timestamp": ts,
        "image_url": f"/static/walkability_overlay.png?ts={ts}",
    }


# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# ENDPOINT: POST /api/walk-route
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
@app.post("/api/walk-route")
def compute_walk_route(req: WalkRouteRequest):
    """
    Geocode origin + destination and compute best walkability-optimized path.
    """
    ...
    from services.surface_temp import fetch_lst_raster
    from services.ndvi import fetch_ndvi_raster
    from services.walkability import compute_walkability_grid, find_best_walk_path

    origin_coords = geocode_location(req.origin)
    if not origin_coords:
        raise HTTPException(
            status_code=400,
            detail=f"Could not geocode origin: '{req.origin}'. Try a more specific Hyderabad address.",
        )
    ...
    if not dest_coords:
        raise HTTPException(
            status_code=400,
            detail=f"Could not geocode destination: '{req.destination}'. Try a more specific Hyderabad address.",
        )

    if not is_within_region(origin_coords[0], origin_coords[1]):
        raise HTTPException(
            status_code=400,
            detail=f"Origin '{origin_coords[2]}' is outside the {cfg.RADIUS_KM}km coverage area around Madhpur.",
        )

    if not is_within_region(dest_coords[0], dest_coords[1]):
        raise HTTPException(
            status_code=400,
            detail=f"Destination '{dest_coords[2]}' is outside the {cfg.RADIUS_KM}km coverage area.",
        )
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail=f"Could not load walkability data: {e}")
    ...
    return {
        ...
        "route": result,
        "walkability_stats": walk_data.get("stats", {}),
        "best_hours": walk_data.get("best_hours", []),
    }
