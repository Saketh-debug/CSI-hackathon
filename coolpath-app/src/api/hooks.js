...
export function useWalkabilityData() {
  return useFetch('/api/walkability')
}
...
export function useWalkRoute() {
  ...
  const compute = useCallback(async (origin, destination) => {
    if (!origin?.trim() || !destination?.trim()) return
    setLoading(true)
    setError(null)
    setResult(null)
    try {
      const r = await client.post('/api/walk-route', {
        origin,
        destination,
      })
      setResult(r.data)
    } catch (e) {
      setError(e.response?.data?.detail || e.message || 'Walk routing failed')
    } finally {
      setLoading(false)
    }
  }, [])
