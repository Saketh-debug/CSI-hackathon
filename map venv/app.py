...
@st.cache_data(ttl=config.TEMP_CACHE_TTL, show_spinner="🚶 Computing walkability...")
def load_walkability_data():
    from services.walkability import compute_walkability_grid
    lst = load_lst_data()
    ndvi = load_ndvi_data()
    return compute_walkability_grid(lst, ndvi)


# ── Main tabs ───────────────────────────────────────────────
tab_temp, tab_canopy, tab_router, tab_walk = st.tabs([
    "🌡️ Temperature Map",
    "🌳 Tree Canopy Map",
    "🛵 CoolPath Router",
    "🚶 WalkWise",
...
# ═══════════════════════════════════════════════════════════
# TAB 4: WalkWise — Best Walking Path
# ═══════════════════════════════════════════════════════════
with tab_walk:
    st.markdown("### 🚶 WalkWise — Best Walking Path")
    st.caption(
        "Finds the safest walking route using 5 metrics: tree shade, heat index, "
        "UV exposure, air quality, and terrain slope."
    )

    wcol1, wcol2 = st.columns(2)
    with wcol1:
        w_origin = st.text_input(
            "🚶 Starting point",
    ...
    with wcol2:
        w_dest = st.text_input(
            "📍 Destination",
            value="KBR Park, Banjara Hills",
            placeholder="e.g., KBR National Park",
            key="walk_dest",
        )

    if st.button("🚶 Find Best Walking Path", type="primary", use_container_width=True, key="find_walk_path"):
        from services.routing import geocode_location, is_within_region
        from services.walkability import find_best_walk_path

        with st.spinner("🔍 Geocoding locations..."):
            o_coords = geocode_location(w_origin)
            d_coords = geocode_location(w_dest)

        if not o_coords:
            st.error(f"❌ Could not find: '{w_origin}'")
        elif not d_coords:
            st.error(f"❌ Could not find: '{w_dest}'")
        elif not is_within_region(o_coords[0], o_coords[1]):
            st.error(f"❌ Starting point is outside the {config.RADIUS_KM}km area.")
        elif not is_within_region(d_coords[0], d_coords[1]):
            st.error(f"❌ Destination is outside the {config.RADIUS_KM}km area.")
        else:
            st.info(f"📍 **{o_coords[2][:60]}** → 🏁 **{d_coords[2][:60]}**")

            with st.spinner("🛰️ Loading walkability data (shade + heat + UV + AQI + slope)..."):
                walk_data = load_walkability_data()

            with st.spinner("🗺️ Computing best walking path..."):
                result = find_best_walk_path(
            ...
            if "error" in result:
                ...
                w_stats = walk_data.get("stats", {})

                # ── 5 Stat cards ──
                sc1, sc2, sc3, sc4, sc5 = st.columns(5)
                with sc1:
                    st.markdown(f"""<div class="stat-card">
                        <h3 style="color:#22c55e">{result['distance_km']} km</h3>
                        <p>📏 Distance</p></div>""", unsafe_allow_html=True)
                with sc2:
                    sc_color = "#10b981" if result['walk_score'] >= 65 else "#f59e0b" if result['walk_score'] >= 40 else "#ef4444"
                    st.markdown(f"""<div class="stat-card">
                        <h3 style="color:{sc_color}">{result['walk_score']}</h3>
                        <p>🏅 Walk Score</p></div>""", unsafe_allow_html=True)
                with sc3:
                    uv_val = w_stats.get("max_uv", 0)
                    uv_color = "#10b981" if uv_val < 3 else "#f59e0b" if uv_val < 6 else "#ef4444"
                    st.markdown(f"""<div class="stat-card">
                        <h3 style="color:{uv_color}">{uv_val}</h3>
                        <p>☀️ UV Index</p></div>""", unsafe_allow_html=True)
                with sc4:
                    aqi_val = w_stats.get("avg_aqi", 0)
                    aqi_color = "#10b981" if aqi_val <= 50 else "#f59e0b" if aqi_val <= 100 else "#ef4444"
                    st.markdown(f"""<div class="stat-card">
                        <h3 style="color:{aqi_color}">{int(aqi_val)}</h3>
                        <p>🌬️ AQI</p></div>""", unsafe_allow_html=True)
                with sc5:
                    slope_val = w_stats.get("avg_slope_deg", 0)
                    st.markdown(f"""<div class="stat-card">
                        <h3 style="color:#64748b">{slope_val}°</h3>
                        <p>⛰️ Slope</p></div>""", unsafe_allow_html=True)


                # ── Map with best path ──
                import folium
                walk_map = create_base_map(zoom=14)

                # Walkability overlay
                walk_bounds = walk_data["bounds"]
                arr_disp = np.clip(np.array(walk_data["grid"]), 0, 100).astype(np.float32)
                import matplotlib
                matplotlib.use('Agg')
                ...
                cmap = mpl_cm.get_cmap('RdYlGn')
                norm = mpl_colors.Normalize(vmin=0, vmax=100)
                rgba = cmap(norm(arr_disp))
                ...
                folium.raster_layers.ImageOverlay(
                    ...
                    bounds=[[walk_bounds[0][0], walk_bounds[0][1]],
                            [walk_bounds[1][0], walk_bounds[1][1]]],
                    opacity=0.5, name="Walkability", show=True,
                ).add_to(walk_map)

                # Best path — green solid
                folium.PolyLine(
                    result["coords"], color="#22c55e", weight=5,
                    tooltip=f"WalkWise: {result['distance_km']}km, score {result['walk_score']}",
                ).add_to(walk_map)

                folium.Marker(
                    result["coords"][0], popup=o_coords[2][:40],
                    icon=folium.Icon(color="blue", icon="play"),
                ).add_to(walk_map)
                folium.Marker(
                    result["coords"][-1], popup=d_coords[2][:40],
                ...
                cm.LinearColormap(
                    ...
                    vmin=0, vmax=100,
                    caption="Walkability (0=worst, 100=best)"
                ).add_to(walk_map)

                folium.LayerControl().add_to(walk_map)
                render_map(walk_map, "walk_path_map")

    else:
        st.info("👆 Enter start and destination, then click **Find Best Walking Path**.")
        default_walk_map = create_base_map()
        import folium
        folium.Marker(
            [config.CENTER_LAT, config.CENTER_LON],
            popup="Madhapur, Hyderabad", icon=folium.Icon(color='green'),
        ).add_to(default_walk_map)
        render_map(default_walk_map, "default_walk_map")
...
<div style="text-align:center; color:#64748b; font-size:0.85rem; padding:1rem 0;">
    <b>CoolPath v3</b> — Climate-Aware Delivery Route Planner + WalkWise Analyzer<br>
    MODIS LST · Sentinel-2 NDVI · OSMnx · Open-Meteo · OpenTopoData SRTM<br>
